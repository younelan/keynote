...
// Scan directory for .knt files
$files = [];
foreach (glob($dataDir . '/*.knt') as $filepath) {
    $filename = basename($filepath);
    $files[$filename] = $filepath;
}

// If no files found, show an error message
if (empty($files)) {
    die('No .knt files found in the data directory. Please add some files to ' . $dataDir);
}

// Determine selected file from GET, defaulting to first available file
$selectedFileKey = isset($_GET['file']) && array_key_exists($_GET['file'], $files) ? $_GET['file'] : array_key_first($files);
